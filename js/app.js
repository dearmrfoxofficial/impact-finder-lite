...
function render(items) {
  ...
  for (const org of items) {
    ...
    if (org.website) {
      ...
      a.href = org.website; a.target = '_blank'; a.rel = 'noopener'; a.className = 'link';
      a.textContent = 'Official Site';
      links.appendChild(a);
    }
    if (org.ein) {
      const a = document.createElement('a');
      a.href = cnUrlFromEIN(org.ein); a.target = '_blank'; a.rel = 'noopener'; a.className = 'link';
      a.textContent = 'Charity Navigator';
      links.appendChild(a);
    }
    const gg = document.createElement('a');
    gg.href = ggSearchUrl(org.name); gg.target = '_blank'; gg.rel = 'noopener'; gg.className = 'link';
    gg.textContent = 'Find Projects on GlobalGiving';
    links.appendChild(gg);
    card.appendChild(h3);
    if (org.cause) {
      const cause = document.createElement('div');
      cause.className = 'pill';
      cause.textContent = org.cause;
...
async function init() {
  try {
    ...
    console.error(e);
    resultMeta.textContent = 'Failed to load data.';
  }
}
